...
class CycleGANModel(nn.Module):
    ...
    def training_cycle(
        ...
        sentences_a_style: str,
        sentences_b_style: str,
        ...
        mono_a = list(sentences_a)
        # Aggiungi il token di stile B a ciascuna frase
        mono_a_with_style = [f"{sentences_b_style} {sentence}" for sentence in mono_a]

        # first half
        out_transferred_ab, transferred_ab = self.G_ab(mono_a_with_style, device=self.device) 
        ...
        # second half
        mono_transferred_ab = list(transferred_ab)
        # Aggiungi il token di stile A a ciascuna frase
        mono_transferred_ab_with_style = [f"{sentences_a_style} {sentence}" for sentence in mono_transferred_ab]
        out_reconstructed_ba, reconstructed_ba, cycle_loss_aba = self.G_ba(mono_transferred_ab_with_style, sentences_a, device=self.device)
        ...
        # ---------- BEGIN : cycle B -> A ----------
        # first half
        mono_b = list(sentences_b)
        # Aggiungi il token di stile A a ciascuna frase
        mono_b_with_style = [f"{sentences_a_style} {sentence}" for sentence in mono_b]
        
        out_transferred_ba, transferred_ba = self.G_ba(mono_b_with_style, device=self.device)
        ...
        # second half
        mono_transferred_ba = list(transferred_ba)
        # Aggiungi il token di stile B a ciascuna frase
        mono_transferred_ba_with_style = [f"{sentences_b_style} {sentence}" for sentence in mono_transferred_ba]
        out_reconstructed_ab, reconstructed_ab, cycle_loss_bab = self.G_ab(mono_transferred_ba_with_style, sentences_b, device=self.device)
