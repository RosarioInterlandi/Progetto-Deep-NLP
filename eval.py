...
class Evaluator():
    ...
    def run_eval_mono(self, epoch, current_training_step, phase, mono_dl_a_eval, mono_dl_b_eval):
        ...
        '''
        ###ONE TO MANY 
        # Definisci il token di stile per lo stile B
        style_token_B = f"<{mono_dl_b_eval.dataset.style}>"

        for batch in mono_dl_a_eval:
            # Converti il batch in una lista di frasi
            mono_a = list(batch)
            ...
            with torch.no_grad():
                # Trasferisci da A a B utilizzando le frasi con il token di stile
                transferred = self.cycleGAN.transfer(sentences=mono_a_with_style, direction='AB')
            
            # Memorizza i risultati per la valutazione
            real_A.extend(mono_a)  # Salva le frasi originali senza il token
            pred_B.extend(transferred)  # Salva le frasi trasferite
            
            # Prepara i dati per il calcolo delle metriche
            mono_a = [[s] for s in mono_a]  # Usa le frasi originali senza token come riferimento
            scores_AB_bleu_self.extend(self.__compute_metric__(transferred, mono_a, 'bleu'))
            scores_rouge_self = np.array(self.__compute_metric__(transferred, mono_a, 'rouge'))
            scores_AB_r1_self.extend(scores_rouge_self[:, 0].tolist())
            scores_AB_r2_self.extend(scores_rouge_self[:, 1].tolist())
        ...
        avg_AB_bleu_self = np.mean(scores_AB_bleu_self)
        avg_AB_r1_self, avg_AB_r2_self, avg_AB_rL_self = (
            np.mean(scores_AB_r1_self),
            np.mean(scores_AB_r2_self),
            np.mean(scores_AB_rL_self),
        )
        
        ###MANY TO ONE  
        # Definisci il token di stile per lo stile A
        style_token_A = f"<{mono_dl_a_eval.dataset.style}>"
        ...
        ###AB and BA AVERAGE METRICS COMPUTATION
    ...
    def run_eval_mono_multistyle(self, epoch, current_training_step, phase, mono_dl_a_eval, mono_dl_b_eval, style_A, style_B):
        """
        Esegue la valutazione su più stili, salva i risultati e li restituisce.
        """
        print(f'Start {phase}...')
        self.cycleGAN.eval()  # set evaluation mode
        if self.args.comet_logging:
